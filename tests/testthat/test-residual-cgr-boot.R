...
boo1 <- bootstrap_mer(m1, mySumm, nsim = NSIM, type = "residual_cgr")
boo2 <- bootstrap_mer(m2, mySumm, nsim = NSIM, type = "residual_cgr")
boo3 <- bootstrap_mer(m3, mySumm, nsim = NSIM, type = "residual_cgr")
boo4 <- bootstrap_mer(m4, mySumm, nsim = NSIM, type = "residual_cgr")


# Return Types ------------------------------------------------------------

test_that("random intercept with residual (CGR) bootstrap", {
  boo <- boo1
  mySumm_m <- mySumm(m1)
...
test_that("random slope with residual (CGR) bootstrap", {
  boo <- boo2
  mySumm_m <- mySumm(m2)
...
test_that("cross-classified with residual (CGR) bootstrap", {
  boo <- boo3
  mySumm_m <- mySumm(m3)
...
test_that("interaction with residual (CGR) bootstrap", {
  boo <- boo4
  mySumm_m <- mySumm(m4)
...
test_that("random intercept with residual (CGR) bootstrap CI", {
  boo <- boo1
...
test_that("random slope with residual (CGR) bootstrap CI", {
  boo <- boo2
  ci_idx <- sample.int(3, size = 1)
  boo_ci <- boot.ci(boo, index = ci_idx, type = c("norm", "basic", "perc"))
  boo_bca <- boot.ci(boo, index = ci_idx, type = "bca",
...
test_that("cross-classified with residual (CGR) bootstrap CI", {
  boo <- boo3
  ci_idx <- sample.int(3, size = 1)
  boo_ci <- boot.ci(boo, index = ci_idx, type = c("norm", "basic", "perc"))
...
test_that("interaction with residual (CGR) bootstrap CI", {
  boo <- boo4
